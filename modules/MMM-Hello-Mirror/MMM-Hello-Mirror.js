	...
	start: function () {
		...
		if (annyang) {
			...
			if (self.config.language == 'de') {
				...
				var commands = {
					'Hello (magic) mirror *command': function (command) {
						Log.info('Voice command recognized in module ' + self.name + ': ' + command);
						if (self.config.broadcastEvents) {
							self.sendNotification("VOICE_COMMAND", command);
							if (command.substring(0, command.length - 1) == "go to page ") {
								var number = command.slice(-1);

								self.sendNotification("PAGE_SELECT", number);
								if (number != "4") {
									self.sendNotification("YT_SHOW_IMAGE");
 
								}
							} else if (command == "play video") {
								self.sendNotification("YT_PLAY_VIDEO");
							}
							else if (command == "stop video") {
								self.sendNotification("YT_SHOW_IMAGE");
							}
							else if (command == "pause video") {
								self.sendNotification("YT_PAUSE_VIDEO");
							}
							else if (command == "resume video") {
								self.sendNotification("YT_UNPAUSE_VIDEO");
							}
						}
						if (responsiveVoice) {
							responsiveVoice.speak(self.translate("VOICE_ACCEPTED"));
